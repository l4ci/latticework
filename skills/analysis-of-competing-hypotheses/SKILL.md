...
A parallel implementation of Analysis of Competing Hypotheses (ACH), the structured analytic technique developed by Richards Heuer at the CIA and laid out in *Psychology of Intelligence Analysis* (1999). ACH exists to defeat one specific failure: the mind picks a favored explanation early, then reads every new fact as confirmation. ACH forces all hypotheses to compete on the same evidence at once, and it ranks them by **disconfirmation** — the most likely hypothesis is the one with the *least evidence against it*, not the one with the most evidence for it.

The skill runs six stages. Stage 1 and the matrix scoring in Stage 3, plus the adversarial check in Stage 5, fan out across parallel subagents. The stages in between are sequential because each depends on the matrix the previous stage produced.
...
Tell each subagent that its final message is the return value, meaning structured data rather than prose for a human. Subagents that gather or test evidence should use whatever retrieval or search tools the runtime offers. If the runtime has no retrieval tools, the agent should say so and lower confidence rather than invent facts.

---

## Stage 1: Hypothesis set (PARALLEL)

The quality of an ACH is capped by the quality of its hypothesis set. A missing hypothesis can never win, so the goal here is near-exhaustiveness, including the explanations nobody wants to be true.

Dispatch three to five proposer subagents in parallel, each told to generate candidate hypotheses for the QUESTION from a different stance, so the set is not all variations on one idea:
...
- Keep it to a workable number, typically three to seven. Do **not** prune a hypothesis yet just because it seems unlikely — ACH's whole point is to test it, and premature rejection is the bias the method is built to catch.
...
This is the core of ACH and the place its discipline lives. Dispatch **one subagent per hypothesis**, in parallel. Each agent sees the full evidence list but is responsible for exactly one column, and is told to **try to disconfirm its hypothesis**, not defend it.
...
Scoring each column with an independent mind committed to disconfirming that one hypothesis is what stops the analyst's favored answer from quietly winning. Output per agent: the column of scores for Hk with rationales.
...
**1. Find the diagnostic evidence; discount the rest.** Read across each row. An item that is Consistent (or N/A) with *every* hypothesis has **no diagnostic value** — it cannot help you choose, however true or important it feels. Flag those rows as non-diagnostic and set them aside from the ranking (note them; do not delete the knowledge). The rows that matter are the ones that are Inconsistent with some hypotheses but not others — those discriminate.

**2. Rank by fewest weighted inconsistencies.** For each hypothesis column, compute an **inconsistency score**: the weighted sum of its I and II cells (II counts double; weight by the evidence item's credibility, so a low-credibility "Inconsistent" counts less). The most likely hypothesis is the one with the **lowest** inconsistency score — the one the evidence fails to disprove — *not* the one with the most C marks. State the ranking with each hypothesis's inconsistency tally and the specific items driving it.
...
- **Rank by disconfirmation, not confirmation.** The winner is the hypothesis with the least evidence *against* it. Evidence "for" a hypothesis is usually consistent with several others and proves little. This inversion is the entire point of ACH; do not quietly revert to counting checkmarks.
- **Diagnostic evidence only.** A fact consistent with every hypothesis has zero power to discriminate, no matter how solid or how central it feels. Identify it and set it aside from the ranking.
- **Hold all hypotheses at once.** Never prune to a favorite early. A hypothesis that is never on the matrix can never be tested, and the one you would have dropped is the one this method is designed to rescue.
- **Score to disprove.** Each column is scored by a mind trying to break that hypothesis, not sell it. That is what neutralizes the analyst's anchor.
- **Absence is evidence.** What you would expect to see but do not is a row in the matrix and is often the most diagnostic one.
- **Surface the linchpins.** Always say how few items the conclusion rests on and what happens if they are wrong or planted. A confident answer hanging on one fragile fact should be reported as exactly that.
- **Report the field.** Give relative likelihoods across all hypotheses and the indicators that would change them. ACH ranks the alternatives; it does not pretend to certainty.
