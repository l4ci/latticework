...
- **Fixed constraint:** no new terminal leases permitted over the horizon. Every priority below must work inside the current six-terminal footprint.
...
|---|---|---|
| **A1** | Raise next-day on-time delivery from 91.5% to 96.0% | B1 |
| **A2** | Cut cost-per-mile from $2.18 to $2.07 (first 5 of the 15 points) | B2 |
| **A3** | Cut empty (deadhead) miles from 22% to 16% of total miles | B2 (and helps B1) |
| **A4** | Cut driver annualized turnover from 34% to 24% | B3 |

Four annual objectives, four parallel cascades. A3 is split out from A2 deliberately: deadhead is the single largest controllable cost lever *and* a reliability lever (idle trucks are trucks not covering late loads), so it earns its own owner and its own catchball rather than hiding inside the cost number.

---

## 2. The X-matrix

This is the headline deliverable. The four legs read clockwise: breakthrough objectives (south), annual objectives (west), improvement priorities (north), targets (east), and the ownership grid (far right). Correlation marks are stated explicitly: **● strong**, **○ weak**, **(blank) none**.
...
|---|---|---|---|---|
| P1 Dynamic dispatch / route-density engine | ● | ○ | ● | |
| P2 Predictive maintenance program | ● | ● | | ○ |
| P3 Backhaul matching / load-board partnership | ○ | ● | ● | |
| P4 Fuel & idle-reduction program | | ● | ○ | |
...
| Priority | Accountable owner | Contributing teams |
|---|---|---|
| P1 Dynamic dispatch engine | VP Operations (Dana Okafor) | Dispatch, IT/Data, Regional Ops |
| P2 Predictive maintenance | Director of Fleet (Marco Reyes) | Maintenance shops, IT/Data |
| P3 Backhaul matching | Director of Network Planning (Lena Sørensen) | Sales, Dispatch |
| P4 Fuel & idle reduction | Director of Fleet (Marco Reyes) | Drivers, Maintenance |
...
|---|---|---|
| P1 Dynamic dispatch engine | Late-load rate ≤ 4.0% on next-day lanes by Q4 FY2026 (from 8.5%) | Dana Okafor |
...
|---|---|---|
| P3 Backhaul matching | Revenue-loaded backhaul share +8 pts by Q4 | Lena Sørensen |
...
| Priority | Target | Owner |
|---|---|---|
| P5 Driver pay & schedule | 90-day new-driver retention ≥ 80% by Q4 (from 58%) | Priya Anand |
...
## 4. Consolidated target list (the East leg / bowling-chart rows)

| Target | Baseline | FY2026 goal | Priority | Owner |
|---|---|---|---|---|
| Next-day on-time % | 91.5% | 96.0% (staged: 93% Q2) | P1, P2, P6 | Okafor |
| Late-load rate | 8.5% | ≤ 4.0% | P1 | Okafor |
| Roadside breakdowns /100k mi | 1.9 | ≤ 0.8 | P2 | Reyes |
| Cost-per-mile | $2.18 | $2.07 | P2, P3, P4 | (B2 owners) |
| Empty-mile ratio | 22% | ≤ 16% | P1, P3 | Okafor / Sørensen |
| Loaded-backhaul share | baseline | +8 pts | P3 | Sørensen |
| Fleet idle time | baseline | −30% | P4 | Reyes |
| Fleet MPG | baseline | +4% | P4 | Reyes |
...
## 5. Alignment check (the golden thread)

Trace every priority and target up to a breakthrough objective, and confirm every breakthrough objective has supporting work below it.

**Upward trace (no orphan priorities):**
...
Every priority reaches a breakthrough objective. Every breakthrough objective (B1, B2, B3) has at least two priorities under it. The thread holds in both directions.

**Orphans and defects found:**
...
These are exactly the items a top-down plan would have dictated and the working level would have quietly disowned. They go back down for one more round before the matrix is committed.
...
**Bowling-chart shape (illustrative, on-time %):**
...
- **Hoshin deploys a strategy; it does not judge whether the strategy is right.** This report assumes "most reliable + 15% cheaper" is the correct bet. Whether the PNW market rewards reliability over price, and whether 15% is achievable without new terminals, are strategic questions upstream of this deployment.
- **Focus depends on holding the vital few.** The discipline that dropped "expand into LTL" must hold all year. Every new idea wants to become a fifth objective; the method's whole value is saying no.
- **The matrix is worth nothing if it is built once and filed.** Its life is the monthly bowling-chart review and the PDCA loop. Without that rhythm, this is wall decoration.
- **A target that is really an activity will not tell you whether you are winning.** Every number in §4 was checked to be a level-and-date, not a to-do. "Certified 100% of dispatchers" passes; "improve dispatcher skills" would not.
- **The two-way catchball is unfinished by design.** Three items (§6) are still in negotiation. Committing the matrix before they resolve would reproduce the top-down dictation the method exists to prevent.
- All names, figures, and claims here are illustrative for documentation purposes and were not researched.
